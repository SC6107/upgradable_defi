...
export const StakeRewards = ({ account, isConnected, onSuccess, }) => {
    ...
    if (error) {
        return (_jsxs("div", { className: "bg-slate-800 rounded-lg border border-slate-700 p-6", children: [_jsx("p", { className: "text-red-400", children: error }), _jsx("button", { onClick: fetchData, className: "mt-2 px-4 py-2 rounded-lg bg-slate-600 text-white text-sm", children: "Retry" })] }));
    }
    if (pools.length === 0) {
